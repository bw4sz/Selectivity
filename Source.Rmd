...
resourceplotS<-ggplot(flowerplot[flowerplot$Species %in% keep,],aes(x=value,linetype=variable,y=Selectivity,col=variable,size=Minutes_Total)) + geom_point() +  stat_smooth(show_guide=F,method="glm",family="binomial",aes(weight=Minutes_Total,group=variable))

resourceplotS+ facet_wrap(~Species,scales="free_x",nrow=3) + labs(x="Available Resources") + theme_gray() + scale_color_manual(values=c("black","grey40"),"Resource Model",labels=c("Favored Resources","All Resources")) + scale_linetype_discrete(guide="none") + labs(size="Minutes") + theme_bw()

ggsave("Figures/Resource_Selectivity.jpeg",dpi=300,height=7,width=10,units="in")
```
...
ggplot(selective.matrix[selective.matrix$Species %in% keep,],aes(x=log(fl_s),y=bph)) + geom_point() + facet_wrap(~Species,scale="free") + geom_smooth(method="glm",family="poisson") + theme_bw() + labs(x="Log Abundance of Preferred Resources",y="Birds per hour")
ggsave("Figures/bphresources.jpg",height=6,width=7,dpi=600)
```
