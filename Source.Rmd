...
library(knitr)
opts_chunk$set(message=FALSE,warning=FALSE,fig.width=13,echo=FALSE,cache=TRUE,cache.path = 'Source_cache/', fig.path='figure/')
...
ggplot(dat,aes(x=seconds(Time_Feeder_Obs))) + geom_histogram()  + theme_bw() + ggtitle("Mean Time Feeding per Species")
```
...
paste("Species feed on the high value", sH/sL, "times than the low value")
...
ggplot(selective.matrix,aes(x=Species,Selectivity,fill=Mass)) + geom_boxplot() + theme_bw() + theme(axis.text.x=element_text(angle=-90,vjust=-.1)) + facet_wrap(~Elevation,scale="free_x",nrow=2) + geom_point()
```
...
resourceplotS<-ggplot(flowerplot[flowerplot$Species %in% keep,],aes(x=value,linetype=variable,y=Selectivity,col=variable,size=Minutes_Total)) + geom_point() +  stat_smooth(method="glm",family="binomial",aes(weight=Minutes_Total,group=variable))
resourceplotS+ facet_wrap(~Species,scales="free_x",nrow=3) + labs(x="Available Resources") + theme_bw() + scale_color_discrete("Resource Model",labels=c("Private Resources","All Resources")) + scale_linetype_discrete(guide="none")
```
...
mod_data<-selective.matrix[selective.matrix$Species %in% keep & complete.cases(selective.matrix[,c("Species","Mass","MassD","Elevation","PhyloD","RDist","MultD","fl_all","fl_s")]), ]

missing<-selective.matrix[!complete.cases(selective.matrix[,c("Species","Mass","MassD","Elevation","MultD","PhyloD", "fl_all","fl_s")]), ]
...
m_phyloD<-glm(data=mod_data,Selectivity~PhyloD:Species,family="binomial",weights=mod_data$Minutes_Total,na.action="na.fail")
...
m_MorphXResources<-glm(data=mod_data,Selectivity~fl_s:Species * Mass:Species,family="binomial",weights=mod_data$Minutes_Total,na.action="na.fail")

m_PhyloXResources<-glm(data=mod_data,Selectivity~fl_s:Species * PhyloD:Species,family="binomial",weights=mod_data$Minutes_Total,na.action="na.fail")

mods<-list(m_intercept,m_species,m_mass,m_massD,m_phyloD,m_multD,m_Elevation,m_Allresources,m_Speciesresources)
...
sumAIC$PsuedoR2<-round(unlist(R2),2)[c(7,8,6,5,2,3,4)]
...
phy_ord<-phy_dist[re_ord,re_oresd]
...
ggsave("Figures/MatrixPlot.jpeg",dpi=600,height=6.5,width=7)
